...
let genAI = null;
export function configurePromptGenerator(apiKey) {
 ...
 */
export async function generateDynamicPrompt(facts, controlBlock, sessionId) {
    const startTime = Date.now();
    if (!genAI) {
        throw new GhostPipelineError('Prompt generator not configured. Call configurePromptGenerator first.', 'CLIENT_NOT_CONFIGURED', 'rendering');
    }
    try {
 ...
 */
function generateFallbackPrompt(facts, controlBlock) {
