 ...
 */
export async function validateAgainstCCJ(generatedImageUrl, flatlayImageUrl, ccj, digest, config = DEFAULT_QA_CONFIG) {
    ...
    try {
        ...
        const colorValidation = await validateColors(generatedImageUrl, flatlayImageUrl, ccj.colors_hex, config);
        // 2. Button count validation (if placket exists)
        const buttonValidation = await validateButtons(generatedImageUrl, ccj.parts.placket?.buttons || 0);
        // 3. Cavity validation (neckline, sleeves must be open)
        const cavityValidation = await validateCavities(generatedImageUrl, ccj.parts, config);
        // 4. Proportions validation
        const proportionValidation = await validateProportions(generatedImageUrl, ccj.proportions, config);
        // Collect errors
 ...
 */
async function validateColors(generatedImageUrl, flatlayImageUrl, expectedColors, config) {
    try {
 ...
 */
async function validateButtons(generatedImageUrl, expectedButtons) {
    try {
 ...
 */
async function validateCavities(generatedImageUrl, parts, config) {
    try {
 ...
 */
async function validateProportions(generatedImageUrl, expectedProportions, config) {
    try {
 ...
 */
export function generateRetryParameters(originalQA) {
...
export function createQAStorageRecord(sessionId, ccj, qaResult, generatedImageUrl, retryAttempt = 0) {
    return {
