 ...
 */
function generateStructuredGhostMannequinPrompt(facts, control) {
 ...
 */
function getFabricBehaviorExamples(material, weave, drapeStiffness) {
    const examples = [];
 ...
 */
function getLightingSetup(lightingPref, shadowBehavior) {
    const setup = [
 ...
 */
function getConstructionDetails(facts, control) {
    const details = [
 ...
 */
function getDetailRefinement(facts, control) {
    const refinements = [];
 ...
 */
export function generateFlashJsonPayload(facts, controlBlock, sessionId, images) {
    try {
...
**JSON SPECIFICATION:**\n${JSON.stringify(structuredPrompt, null, 2)}`;
        ...
        throw new GhostPipelineError(`Failed to generate JSON payload: ${error instanceof Error ? error.message : 'Unknown error'}`, 'JSON_PAYLOAD_FAILED', 'rendering', error instanceof Error ? error : undefined);
    }
}
/**
 * Helper function to convert region hints from Record<string, string[]> to Record<string, string>
 */
function convertRegionHintsToStringRecord(regionHints) {
    if (!regionHints)
        return {};
    const result = {};
...
function mapCategoryGeneric(category) {
    const categoryMap = {
        'top': 'top',
        'bottom': 'bottom',
        'dress': 'dress',
...
function mapTransparency(transparency) {
...
function mapSurfaceSheen(sheen) {
    const sheenMap = {
        'matte': 'matte',
...
function mapShadowStyle(shadow) {
    const shadowMap = {
        'soft': 'soft',
        'medium': 'medium',
        'hard': 'hard'
...
function mapLabelVisibility(visibility) {
    return visibility === 'optional' ? 'optional' : 'required';
}
function mapLightingPreference(control) {
...
function mapShadowBehavior(control) {
...
function mapDetailSharpness(control) {
    // Enhanced sharpness for ghost mannequin detail preservation
    return 'sharp';
}
function mapTextureEmphasis(control) {
    // Enhance texture for better fabric representation
    return 'enhance';
}
function extractHollowRegions(control) {
