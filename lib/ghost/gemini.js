...
import { AnalysisJSONSchema, AnalysisJSONSchemaObject, EnrichmentJSONSchema, EnrichmentJSONSchemaObject, GhostPipelineError, ANALYSIS_PROMPT, ENRICHMENT_ANALYSIS_PROMPT, GHOST_MANNEQUIN_PROMPT } from "@/types/ghost";
// Initialize Gemini client
let genAI = null;
export function configureGeminiClient(apiKey) {
 ...
 */
async function prepareImageForGemini(imageInput, maxDimension = 1024) {
    ...
    if (imageInput.startsWith('data:image/')) {
        ...
        try {
            ...
            throw new GhostPipelineError(`Failed to fetch image from URL: ${error instanceof Error ? error.message : 'Unknown error'}`, 'IMAGE_FETCH_FAILED', 'analysis', error instanceof Error ? error : undefined);
        }
    }
    else {
 ...
 */
function getImageMimeType(imageInput) {
    if (imageInput.startsWith('data:image/')) {
 ...
 */
export async function analyzeGarment(imageUrl, sessionId) {
    const startTime = Date.now();
    if (!genAI) {
        throw new GhostPipelineError('Gemini client not configured. Call configureGeminiClient first.', 'CLIENT_NOT_CONFIGURED', 'analysis');
    }
    try {
        ...
        if (error instanceof Error) {
            if (error.message.includes('quota') || error.message.includes('rate limit')) {
                throw new GhostPipelineError('Gemini API quota exceeded or rate limit hit', 'GEMINI_QUOTA_EXCEEDED', 'analysis', error);
            }
            if (error.message.includes('safety') || error.message.includes('blocked')) {
                throw new GhostPipelineError('Content blocked by Gemini safety filters', 'CONTENT_BLOCKED', 'analysis', error);
            }
        }
        throw new GhostPipelineError(`Garment analysis failed: ${error instanceof Error ? error.message : 'Unknown error'}`, 'ANALYSIS_FAILED', 'analysis', error instanceof Error ? error : undefined);
    }
}
/**
 ...
 */
export async function analyzeGarmentEnrichment(imageUrl, sessionId, baseAnalysisSessionId) {
    const startTime = Date.now();
    if (!genAI) {
        throw new GhostPipelineError('Gemini client not configured. Call configureGeminiClient first.', 'CLIENT_NOT_CONFIGURED', 'analysis');
    }
    try {
        ...
        throw new GhostPipelineError(`Garment enrichment analysis failed: ${error instanceof Error ? error.message : 'Unknown error'}`, 'ANALYSIS_FAILED', 'analysis', error instanceof Error ? error : undefined);
    }
}
/**
 * Try analysis with structured output (responseSchema) - WORKING VERSION FROM COMMIT
 */
async function analyzeWithStructuredOutput(imageUrl, sessionId) {
    const startTime = Date.now();
 ...
 */
async function analyzeWithFallbackMode(imageUrl, sessionId) {
    const startTime = Date.now();
 ...
 */
function createMinimalAnalysis(sessionId) {
    return {
        type: "garment_analysis",
        meta: {
            schema_version: "4.1",
 ...
 */
function ensureRequiredFields(response, sessionId) {
    return {
 ...
 */
export async function generateGhostMannequin(flatlayImage, analysis, originalImage, enrichment) {
    const startTime = Date.now();
    let baseAnalysisFilePath = null;
    let enrichmentAnalysisFilePath = null;
    let fs;
    if (!genAI) {
        throw new GhostPipelineError('Gemini client not configured. Call configureGeminiClient first.', 'CLIENT_NOT_CONFIGURED', 'rendering');
    }
    try {
        ...
        let enrichmentAnalysisJsonBase64 = null;
        if (enrichment) {
        ...
        if (error instanceof Error) {
            if (error.message.includes('quota') || error.message.includes('rate limit')) {
                throw new GhostPipelineError('Gemini API quota exceeded or rate limit hit', 'GEMINI_QUOTA_EXCEEDED', 'rendering', error);
            }
            if (error.message.includes('safety') || error.message.includes('blocked')) {
                throw new GhostPipelineError('Content blocked by Gemini safety filters', 'CONTENT_BLOCKED', 'rendering', error);
            }
        }
        throw new GhostPipelineError(`Ghost mannequin generation failed: ${error instanceof Error ? error.message : 'Unknown error'}`, 'RENDERING_FAILED', 'rendering', error instanceof Error ? error : undefined);
    }
}
/**
 ...
 */
export async function generateEnhancedGhostMannequin(flatlayImage, analysis, onModelImage) {
 ...
 */
export async function generateGhostMannequinWithControlBlock(flatlayImage, controlBlockPrompt, consolidation, originalImage) {
    const startTime = Date.now();
    ...
    try {
        ...
        throw new GhostPipelineError(`Control Block Seedream generation failed: ${error instanceof Error ? error.message : 'Unknown error'}`, 'RENDERING_FAILED', 'rendering', error instanceof Error ? error : undefined);
    }
}
/**
 ...
 * @param originalImage - Optional on-model reference
 */
export async function generateGhostMannequinWithControlBlockGemini(flatlayImage, controlBlockPrompt, consolidation, originalImage) {
    const startTime = Date.now();
    ...
    try {
        ...
        const result = await generateImageWithFreepikGemini(controlBlockPrompt, flatlayImage, // Cleaned flatlay from FAL
        ...
        throw new GhostPipelineError(`Freepik Gemini generation failed: ${error instanceof Error ? error.message : 'Unknown error'}`, 'RENDERING_FAILED', 'rendering', error instanceof Error ? error : undefined);
    }
}
export async function generateGhostMannequinWithSeedream(flatlayImage, analysis, originalImage, enrichment) {
    const startTime = Date.now();
    ...
    try {
        ...
        if (error instanceof Error) {
            if (error.message.includes('quota') || error.message.includes('rate limit')) {
                throw new GhostPipelineError('FAL.AI API quota exceeded or rate limit hit', 'FAL_QUOTA_EXCEEDED', 'rendering', error);
            }
            if (error.message.includes('safety') || error.message.includes('blocked')) {
                throw new GhostPipelineError('Content blocked by FAL.AI safety filters', 'CONTENT_BLOCKED', 'rendering', error);
            }
        }
        throw new GhostPipelineError(`Seedream ghost mannequin generation failed: ${error instanceof Error ? error.message : 'Unknown error'}`, 'RENDERING_FAILED', 'rendering', error instanceof Error ? error : undefined);
    }
}
/**
 * Fallback ghost mannequin generation when Gemini image generation fails
 */
async function generateFallbackGhostMannequin(flatlayImage, analysis) {
 ...
 */
export async function testGeminiFlashImageGeneration() {
    if (!genAI) {
        return false;
    }
    try {
        ...
        if (response.candidates && response.candidates.length > 0) {
            const candidate = response.candidates[0];
            if (candidate.content && candidate.content.parts) {
                const hasImage = candidate.content.parts.some(part => part.inlineData && part.inlineData.mimeType && part.inlineData.mimeType.startsWith('image/'));
 ...
 */
export async function validateGeminiApi() {
    if (!genAI) {
        return false;
    }
    try {
 ...
 */
async function analyzeEnrichmentWithStructuredOutput(imageUrl, sessionId, baseAnalysisSessionId) {
    const startTime = Date.now();
 ...
 */
async function analyzeEnrichmentWithFallbackMode(imageUrl, sessionId, baseAnalysisSessionId) {
    const startTime = Date.now();
 ...
 */
function createMinimalEnrichmentAnalysis(sessionId, baseAnalysisSessionId) {
    return {
        type: "garment_enrichment_focused",
        meta: {
            schema_version: "4.3",
 ...
 */
function ensureRequiredEnrichmentFields(response, sessionId, baseAnalysisSessionId) {
    return {
