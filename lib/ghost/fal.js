...
export function configureFalClient(apiKey) {
    fal.config({
        credentials: apiKey,
    });
}
...
export async function removeBackground(imageUrl) {
    ...
    try {
        // Validate input
        if (!imageUrl) {
            throw new GhostPipelineError('Image URL is required for background removal', 'MISSING_IMAGE_URL', 'background_removal');
        }
        // Prepare request
        const request = {
            image_url: imageUrl,
        ...
        if (!resultImageUrl || typeof resultImageUrl !== 'string') {
            ...
            throw new GhostPipelineError('Invalid response from FAL.AI: missing image URL', 'INVALID_FAL_RESPONSE', 'background_removal');
        }
        ...
        if (error instanceof Error) {
            if (error.message.includes('rate limit')) {
                throw new GhostPipelineError('FAL.AI rate limit exceeded. Please try again later.', 'RATE_LIMIT_EXCEEDED', 'background_removal', error);
            }
            if (error.message.includes('insufficient credits')) {
                throw new GhostPipelineError('Insufficient FAL.AI credits', 'INSUFFICIENT_CREDITS', 'background_removal', error);
            }
            if (error.message.includes('invalid image')) {
                throw new GhostPipelineError('Invalid image format or corrupted image', 'INVALID_IMAGE_FORMAT', 'background_removal', error);
            }
        }
        ...
        throw new GhostPipelineError(`Background removal failed: ${error instanceof Error ? error.message : 'Unknown error'}`, 'BACKGROUND_REMOVAL_FAILED', 'background_removal', error instanceof Error ? error : undefined);
    }
}
/**
 ...
 */
export async function getEstimatedProcessingTime(imageUrl) {
    try {
 ...
 */
export async function uploadImageToFalStorage(imageDataUrl) {
    try {
