...
export function configureAiStudioClient(apiKey) {
 ...
 */
async function uploadImageToFilesAPI(imageInput, role, sessionId) {
    if (!genAI) {
        throw new GhostPipelineError('AI Studio client not configured. Call configureAiStudioClient first.', 'CLIENT_NOT_CONFIGURED', 'rendering');
    }
    ...
    try {
        ...
        throw new GhostPipelineError(`Failed to upload ${role} image to Files API: ${error instanceof Error ? error.message : 'Unknown error'}`, 'FILE_UPLOAD_FAILED', 'rendering', error instanceof Error ? error : undefined);
    }
}
/**
 ...
 */
async function prepareImageForAiStudio(imageInput, options = {}) {
    // Smart defaults based on image role for token optimization
    const { maxWidth = options.role === 'flatlay' ? 1024 : 768, // Flatlay needs more detail
    ...
    if (imageInput.startsWith('data:image/')) {
        ...
        try {
            ...
            throw new GhostPipelineError(`Failed to fetch image from URL: ${error instanceof Error ? error.message : 'Unknown error'}`, 'IMAGE_FETCH_FAILED', 'rendering', error instanceof Error ? error : undefined);
        }
    }
    else {
 ...
 */
function optimizePayloadForTokens(factsV3, controlBlock) {
    // Very gentle cleanup - only remove clearly empty values
    function lightClean(obj) {
        if (Array.isArray(obj)) {
            return obj.filter(item => item !== null &&
                item !== undefined &&
                item !== '' &&
                !(typeof item === 'object' && item !== null && Object.keys(item).length === 0)).map(lightClean);
        }
 ...
 */
export async function generateGhostMannequinWithStructuredJSON(flatlayImage, factsV3, controlBlock, originalImage, options) {
    ...
    try {
        ...
        if (!response) {
            throw new GhostPipelineError('No response received from AI Studio after retries', 'RENDERING_FAILED', 'rendering');
        }
        const candidates = response.candidates;
        if (!candidates?.[0]?.content?.parts) {
            throw new GhostPipelineError('AI Studio response contains no content', 'RENDERING_FAILED', 'rendering');
        }
        const imagePart = candidates[0].content.parts.find(part => part.inlineData?.mimeType?.startsWith('image/'));
        if (!imagePart?.inlineData) {
            throw new GhostPipelineError('AI Studio did not generate an image', 'RENDERING_FAILED', 'rendering');
        }
        ...
        if (error instanceof Error && error.message.includes('429') && error.message.includes('quota')) {
            throw new GhostPipelineError(`Gemini API rate limit exceeded. ${error.message}`, 'GEMINI_QUOTA_EXCEEDED', 'rendering', error);
        }
        throw new GhostPipelineError(`AI Studio embedded JSON generation failed: ${error instanceof Error ? error.message : 'Unknown error'}`, 'RENDERING_FAILED', 'rendering', error instanceof Error ? error : undefined);
    }
}
/**
 * Generate ghost mannequin using AI Studio (LEGACY - uses text prompts)
 */
export async function generateGhostMannequinWithAiStudio(flatlayImage, consolidation, originalImage, sessionId, options) {
    const startTime = Date.now();
    if (!genAI) {
        throw new GhostPipelineError('AI Studio client not configured. Call configureAiStudioClient first.', 'CLIENT_NOT_CONFIGURED', 'rendering');
    }
    ...
    try {
        ...
        if (options?.useStructuredPrompt) {
            ...
            const structuredPrompt = await buildDynamicFlashPrompt(consolidation.facts_v3, consolidation.control_block, sessionId || 'ai-studio-gen', options.useStructuredPrompt, options.useExpertPrompt);
            promptResult = {
            ...
            promptResult = await generateDynamicPrompt(consolidation.facts_v3, consolidation.control_block, sessionId || 'ai-studio-gen');
        }
        ...
        if (candidates && candidates.length > 0) {
            ...
            throw new GhostPipelineError('AI Studio response contains no candidates', 'RENDERING_FAILED', 'rendering');
        }
        const processingTime = Date.now() - startTime;
        ...
        if (error instanceof Error) {
            if (error.message.includes('quota') || error.message.includes('rate limit')) {
                ...
                throw new GhostPipelineError('AI Studio API quota exceeded - retry also failed', 'GEMINI_QUOTA_EXCEEDED', 'rendering', error);
            }
            if (error.message.includes('safety') || error.message.includes('blocked')) {
                throw new GhostPipelineError('Content blocked by AI Studio safety filters', 'CONTENT_BLOCKED', 'rendering', error);
            }
        }
        throw new GhostPipelineError(`AI Studio ghost mannequin generation failed: ${error instanceof Error ? error.message : 'Unknown error'}`, 'RENDERING_FAILED', 'rendering', error instanceof Error ? error : undefined);
    }
}
/**
 ...
 */
export async function generateGhostMannequinWithAiStudioSimple(flatlayImage, consolidation, originalImage, sessionId) {
    const startTime = Date.now();
    if (!genAI) {
        throw new GhostPipelineError('AI Studio client not configured. Call configureAiStudioClient first.', 'CLIENT_NOT_CONFIGURED', 'rendering');
    }
...
Preserve every original color, pattern, and design element exactly as shown in the reference image. Maintain any visible brand labels, care labels, or text elements with perfect clarity and readability.`;
        ...
        throw new GhostPipelineError('AI Studio simple generation failed to produce image', 'RENDERING_FAILED', 'rendering');
    }
    catch (error) {
        console.error('❌ AI Studio simple generation failed:', error);
        throw error instanceof GhostPipelineError ? error : new GhostPipelineError(`AI Studio simple generation failed: ${error instanceof Error ? error.message : 'Unknown error'}`, 'RENDERING_FAILED', 'rendering', error instanceof Error ? error : undefined);
    }
}
/**
 * Health check for AI Studio integration
 */
export async function checkAiStudioHealth() {
    if (!genAI) {
        return {
            status: 'error',
