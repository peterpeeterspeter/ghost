...
import { consolidateAnalyses, buildDynamicFlashPrompt, buildStaticFlashPrompt, buildSeeDreamPrompt, qaLoop } from './consolidation';
import { configureFalClient, removeBackground } from './fal';
import { configureGeminiClient, analyzeGarment, analyzeGarmentEnrichment, generateGhostMannequinWithControlBlock } from './gemini';
import { configureAiStudioClient, generateGhostMannequinWithStructuredJSON } from './ai-studio';
/**
...
export class GhostMannequinPipeline {
    ...
    initializeClients() {
        try {
            ...
            throw new GhostPipelineError('Failed to initialize API clients', 'CLIENT_INITIALIZATION_FAILED', 'background_removal', error instanceof Error ? error : undefined);
        }
    }
    /**
    ...
    async process(request) {
        try {
            ...
            await this.executeStage('background_removal', async () => {
                ...
                const flatlayResult = await this.executeWithTimeout(removeBackground(request.flatlay), this.options.timeouts.backgroundRemoval, 'background_removal');
                ...
                if (process.env.ENABLE_EARLY_FILES_UPLOAD !== 'false') {
                    try {
                        ...
                        const filesApiUri = await this.uploadImageToFilesAPI(flatlayResult.cleanedImageUrl, 'flatlay', this.state.sessionId);
            ...
            await this.executeStage('analysis', async () => {
                ...
                const result = await this.executeWithTimeout(analyzeGarment(imageReference, this.state.sessionId), this.options.timeouts.analysis, 'analysis');
            ...
            await this.executeStage('enrichment', async () => {
                ...
                const result = await this.executeWithTimeout(analyzeGarmentEnrichment(imageReference, enrichmentSessionId, baseAnalysisSessionId), this.options.timeouts.enrichment, 'enrichment');
            ...
            await this.executeStage('consolidation', async () => {
                ...
                if (shouldUseCCJPipeline()) {
                    ...
                    const ccjResult = await this.executeWithTimeout(integrateCCJWithExistingPipeline(analysis, enrichmentData, {
                    ...
                    const result = await this.executeWithTimeout(consolidateAnalyses(analysis, enrichmentData, {
                        cleanedImageUrl: cleanedGarmentDetail,
                        onModelUrl: cleanedOnModel
                    }, this.state.sessionId), this.options.timeouts.consolidation, 'consolidation');
                ...
                await this.executeStage('rendering', async () => {
                    ...
                    if (!userRequestedStructured && renderingApproach === 'optimized') {
                        ...
                        try {
                            ...
                            const result = await this.executeWithTimeout(this.generateWithOptimizedJSON(consolidation), this.options.timeouts.rendering, 'rendering');
                        ...
                        try {
                            ...
                            const result = await this.executeWithTimeout(this.generateWithJsonPayload(consolidation, this.options.renderingModel), this.options.timeouts.rendering, 'rendering');
                            this.state.stageResults.rendering = result;
                            return result;
                        }
                        catch (jsonError) {
                    ...
                    if (userRequestedStructured) {
                        ...
                        promptToUse = await buildDynamicFlashPrompt(consolidation.facts_v3, consolidation.control_block, this.state.sessionId, this.state.originalRequest?.options?.useStructuredPrompt, this.state.originalRequest?.options?.useExpertPrompt);
                    ...
                    const result = await this.executeWithTimeout(this.generateWithControlBlock(promptToUse, consolidation), this.options.timeouts.rendering, 'rendering');
                    this.state.stageResults.rendering = result;
                    return result;
                });
            }
            ...
            if (this.options.enableQaLoop && this.state.stageResults.rendering) {
                await this.executeStage('qa', async () => {
                    ...
                    while (iterations < this.options.maxQaIterations) {
                        const qaReport = await this.executeWithTimeout(qaLoop(currentImageUrl, factsV3, this.state.sessionId), this.options.timeouts.qa, 'qa');
            ...
            this.state.error = error instanceof GhostPipelineError ? error : new GhostPipelineError(`Pipeline failed: ${error instanceof Error ? error.message : 'Unknown error'}`, 'PIPELINE_FAILED', this.state.currentStage || 'background_removal', error instanceof Error ? error : undefined);
     ...
     */
    async executeStage(stage, executor) {
     ...
     */
    async executeWithTimeout(promise, timeout, stage) {
        return new Promise((resolve, reject) => {
            const timer = setTimeout(() => {
                reject(new GhostPipelineError(`Stage ${stage} timed out after ${timeout}ms`, 'STAGE_TIMEOUT', stage));
            }, timeout);
            promise
                .then((result) => {
                clearTimeout(timer);
    ...
    async validateRequest(request) {
        if (!request.flatlay) {
            throw new GhostPipelineError('Flatlay image is required', 'MISSING_FLATLAY', 'background_removal');
        }
    ...
    buildResult() {
        ...
        if (stageResults.analysis) {
            result.analysis = stageResults.analysis.analysis;
        }
        ...
        if (stageResults.qaReport) {
            result.qa_report = stageResults.qaReport;
        }
     ...
     */
    async generateWithOptimizedJSON(consolidation) {
        const cleanedGarmentDetail = this.state.stageResults.backgroundRemovalFlatlay.cleanedImageUrl;
        ...
        try {
            ...
            const result = await generateWithOptimizedJSON(analysis, enrichment, {
     ...
     */
    async generateWithJsonPayload(consolidation, renderingModel) {
        const cleanedGarmentDetail = this.state.stageResults.backgroundRemovalFlatlay.cleanedImageUrl;
        const originalOnModel = this.state.originalRequest?.onModel;
        ...
        try {
            ...
            const jsonPayload = generateFlashJsonPayload(consolidation.facts_v3, consolidation.control_block, this.state.sessionId, {
                flatlayUrl: cleanedGarmentDetail,
                onModelUrl: originalOnModel
            });
            // Validate payload
            ...
            const result = await generateGhostMannequinWithJsonPayload(jsonPayload, renderingModel, consolidation // Pass original consolidation to preserve full FactsV3 data
            );
     ...
     */
    async generateWithControlBlock(controlBlockPrompt, consolidation) {
        const cleanedGarmentDetail = this.state.stageResults.backgroundRemovalFlatlay.cleanedImageUrl;
        const cleanedOnModel = this.state.stageResults.backgroundRemovalOnModel?.cleanedImageUrl;
        ...
        switch (this.options.renderingModel) {
            case 'seedream':
                ...
                return await generateGhostMannequinWithControlBlock(cleanedGarmentDetail, controlBlockPrompt, consolidation, cleanedOnModel);
            case 'ai-studio':
                // AI Studio uses OPTIMAL direct JSON approach (no text conversion)
                this.log('🎯 Using AI Studio with direct JSON payload (OPTIMAL)');
                try {
                    return await generateGhostMannequinWithStructuredJSON(cleanedGarmentDetail, consolidation.facts_v3, // Direct JSON from Gemini Pro analysis
            ...
            case 'freepik-gemini':
                ...
                throw new GhostPipelineError('Freepik rendering disabled for cost control. Use ai-studio instead.', 'FREEPIK_DISABLED', 'rendering');
            case 'gemini-flash':
            default:
                // Fallback to ai-studio for safety
                this.log('⚠️ Using ai-studio fallback for legacy gemini-flash');
                return await generateGhostMannequinWithStructuredJSON(cleanedGarmentDetail, consolidation.facts_v3, consolidation.control_block, originalOnModel, { sessionId: this.state.sessionId });
        }
    }
    /**
     ...
     */
    async uploadImageToFilesAPI(imageUrl, role, sessionId) {
        try {
            ...
            throw new GhostPipelineError(`Failed to upload ${role} image to Files API: ${error instanceof Error ? error.message : 'Unknown error'}`, 'FILES_API_UPLOAD_FAILED', 'background_removal', error instanceof Error ? error : undefined);
        }
    }
    /**
 ...
 */
export async function processGhostMannequin(request, options) {
 ...
 */
export async function processBatch(requests, options, concurrency = 3) {
    const results = [];
    const chunks = [];
