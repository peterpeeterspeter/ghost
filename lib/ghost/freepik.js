 ...
 */
async function prepareImageForFreepik(imageInput, useDirectUrls = true) {
 ...
 */
async function createGeminiTask(prompt, inputImage, referenceImage, apiKey) {
    ...
    const images = [await prepareImageForFreepik(inputImage, useDirectUrls)];
    ...
    const debugPayload = {
        ...payload,
        reference_images: payload.reference_images.map((img, idx) => `base64_data_${img.length}_bytes_image_${idx}`)
    };
    ...
    if (!response.ok) {
        ...
        console.error('❌ Request payload that failed:', JSON.stringify({ ...payload, reference_images: payload.reference_images.map((img, idx) => `[base64_${img.length}bytes_img${idx}]`) }, null, 2));
 ...
 */
async function pollTaskCompletion(taskId, apiKey, maxWaitTime = 300000, // 5 minutes
 ...
 */
export async function testFreepikImageGeneration(testImage) {
 ...
 */
export async function testFreepikWithSimplePrompt(imageUrl1, imageUrl2) {
    const apiKey = process.env.FREEPIK_API_KEY_DISABLED;
    if (!apiKey)
        throw new Error('Missing API key');
...
export async function generateImageWithFreepikGemini(prompt, inputImage, referenceImage) {
    const startTime = Date.now();
    const apiKey = process.env.FREEPIK_API_KEY_DISABLED;
    if (!apiKey) {
        throw new GhostPipelineError('Freepik API key not configured', 'CONFIGURATION_ERROR', 'rendering');
    }
    ...
    try {
        ...
        throw new GhostPipelineError(`Freepik generation failed: ${error instanceof Error ? error.message : 'Unknown error'}`, 'RENDERING_FAILED', 'rendering', error instanceof Error ? error : undefined);
    }
}
/**
 ...
 */
export async function generateImageWithFreepikGeminiJson(jsonPrompt, inputImage, referenceImage) {
    const startTime = Date.now();
    const apiKey = process.env.FREEPIK_API_KEY_DISABLED;
    if (!apiKey) {
        throw new GhostPipelineError('Freepik API key not configured', 'CONFIGURATION_ERROR', 'rendering');
    ...
    try {
        ...
        throw new GhostPipelineError(`Freepik Gemini JSON generation failed: ${error instanceof Error ? error.message : 'Unknown error'}`, 'RENDERING_FAILED', 'rendering', error instanceof Error ? error : undefined);
    }
}
/**
 ...
 */
export async function generateImageWithFreepikMystic(prompt, inputImage, referenceImage) {
    const startTime = Date.now();
    const apiKey = process.env.FREEPIK_API_KEY_DISABLED;
    if (!apiKey) {
        throw new GhostPipelineError('Freepik API key not configured', 'CONFIGURATION_ERROR', 'rendering');
    ...
    try {
        ...
        if (!response.ok) {
            ...
            throw new GhostPipelineError(`Freepik Mystic API error: ${response.status} - ${errorText}`, 'RENDERING_FAILED', 'rendering');
        }
        const result = await response.json();
        const processingTime = Date.now() - startTime;
        ...
        if (result && result.output) {
            ...
            if (typeof result.output === 'string' && result.output.startsWith('http')) {
                ...
                throw new GhostPipelineError('Unexpected Mystic API response format', 'RENDERING_FAILED', 'rendering');
            }
            return {
                imageBase64,
                processingTime,
            ...
            throw new GhostPipelineError('No images generated by Freepik Mystic', 'RENDERING_FAILED', 'rendering');
        }
    }
    catch (error) {
        const processingTime = Date.now() - startTime;
        ...
        throw new GhostPipelineError(`Freepik Mystic generation failed: ${error instanceof Error ? error.message : 'Unknown error'}`, 'RENDERING_FAILED', 'rendering', error instanceof Error ? error : undefined);
    }
}
/**
 * Health check for Freepik API
 */
export async function checkFreepikHealth() {
    const apiKey = process.env.FREEPIK_API_KEY_DISABLED;
    if (!apiKey) {
        return {
            status: 'error',
