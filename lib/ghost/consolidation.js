...
const HexLoose = z.preprocess((v) => (v === null || v === undefined ? undefined : v), Hex.optional());
export const PaletteSchemaLoose = z.object({
    ...
    region_hints: z.preprocess((v) => normalizeRegionHints(v), z.record(z.array(z.string()))).optional(),
});
/** accept object OR array OR null and coerce to { must_not: [...] } */
const SafetySchemaLoose = z.preprocess((v) => {
    if (v === null || v === undefined)
        return { must_not: [] };
    if (Array.isArray(v))
        return { must_not: v };
    if (typeof v === 'object' && v !== null) {
        return { must_not: Array.isArray(v.must_not) ? v.must_not : [] };
    }
    return { must_not: [] };
...
export const QAReportSchema = z.object({
    overall_quality_score: z.preprocess((v) => typeof v === 'string' ? parseFloat(v) : v, z.number().min(0).max(1).default(0)),
    deltas: z.array(z.object({
        metric: z.string(),
        current_value: z.preprocess((v) => typeof v === 'string' ? parseFloat(v) : v, z.number()),
        target_value: z.preprocess((v) => typeof v === 'string' ? parseFloat(v) : v, z.number()),
        correction_prompt: z.string(),
    })).default([]),
    passed: z.preprocess((v) => typeof v === 'string' ? v.toLowerCase() === 'true' : v, z.boolean().default(false)),
});
...
// -----------------------------
function coerceHex(hex, ...fallbacks) {
...
function normalizeSafety(s) {
    if (Array.isArray(s))
        return { must_not: s };
    if (s && Array.isArray(s.must_not))
        return { must_not: s.must_not };
    return { must_not: [] };
}
...
export function normalizeControlBlock(c, factsFallback) {
...
// Core Logic
// -----------------------------
export async function consolidateAnalyses(jsonA, jsonB, refs, sessionId) {
  ...
  "conflicts_found": []
    ...
    try {
        ...
        if (factsResult.success) {
            ...
            console.log('Schema validation errors:', factsResult.error.issues.map(issue => `${issue.path.join('.')}: ${issue.message}`).join('; '));
            ...
            recoveredFacts.palette = {
                ...
                pattern_hexes: Array.isArray(rawPalette.pattern_hexes)
                    ? rawPalette.pattern_hexes.filter((h) => typeof h === 'string' && /^#[0-9A-Fa-f]{6}$/.test(h))
                    : [],
        ...
        if (controlResult.success) {
            ...
            console.log('Control block validation errors:', controlResult.error.issues.map(issue => `${issue.path.join('.')}: ${issue.message}`).join('; '));
...
export function compileControlBlock(facts) {
 ...
 */
export async function buildDynamicFlashPrompt(facts, control, sessionId, useStructuredPrompt, useExpertPrompt) {
    try {
 ...
 */
export function buildStaticFlashPrompt(control) {
    ...
    return ('\nTask: Using the provided reference images, create a professional studio product photo with invisible mannequin effect (ghost mannequin). Transform the flat-laid garment from the input images into a 3D ghost mannequin effect that shows exactly the same garment design, colors, patterns, and details. No human body or model visible.' +
        ...
        '\n- Preserve ALL original colors, patterns, textures, and design elements from the reference images').trim();
}
/**
 * Legacy alias for backwards compatibility
 */
export function buildFlashPrompt(control) {
 ...
 */
export function buildSeeDreamPrompt(control, facts) {
    return buildStaticFlashPrompt(control);
}
export async function qaLoop(imageUrl, facts, sessionId) {
