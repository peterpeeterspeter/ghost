...
export async function generateGhostMannequinWithControlBlockGemini(
  ...
  console.log('Starting ghost mannequin generation with Control Block + Gemini Flash...');
  ...
  try {
    ...
    // Add on-model reference if provided
    if (originalImage) {
      const originalImageData = await prepareImageForGemini(originalImage);
      const originalMimeType = getImageMimeType(originalImage);
      parts.push({
        inlineData: {
    ...
    console.log('Calling Gemini Flash with Control Block...');
      ...
      if (candidate.content && candidate.content.parts) {
        for (const part of candidate.content.parts) {
          if (part.inlineData && part.inlineData.data) {
            ...
            const dataUri = `data:${part.inlineData.mimeType || 'image/png'};base64,${imageData}`;
            
            const processingTime = Date.now() - startTime;
            console.log(`Control Block Gemini generation completed in ${processingTime}ms`);
    ...
    throw new Error('No image generated by Gemini Flash with Control Block');
    
  } catch (error) {
    const processingTime = Date.now() - startTime;
    console.error('Control Block Gemini generation failed:', error);
    
    throw new GhostPipelineError(
      `Control Block Gemini generation failed: ${error instanceof Error ? error.message : 'Unknown error'}`,
      'RENDERING_FAILED',
      'rendering',
      error instanceof Error ? error : undefined
    );
